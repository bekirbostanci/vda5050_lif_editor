  ...
  <div>
    ...
    <div class="ml-auto flex w-full space-x-2 py-2">
      <div
        ...
        v-if="props.sidebar.selectedStations.value.length > 0"
      >
        ...
        <Input
          ...
          type="number"
          v-model="
            props.layout.layouts.nodes[props.sidebar.selectedStations.value[0]]
              .x
          "
          :default-value="0"
        />
      ...
      <div class="grid gap-2 mt-2" v-else>
        ...
        <Input
          id="station-x"
          type="number"
          v-model="props.sidebarStation.newStation.value.stationPosition.x"
          :default-value="0"
        />
      </div>
      <div
      ...
      >
        ...
        <Input
          ...
          :default-value="0"
          @update:model-value="
            props.sidebarStation.updateStationYFromInput($event)
          "
          v-model="
            props.layout.nodes[props.sidebar.selectedStations.value[0]]
              .vda5050Station!.stationPosition.y
          "
        />
      </div>
      <div class="grid gap-2 mt-2" v-else>
      ...
      <div class="grid gap-2 mt-2">
        ...
        <Input
          id="station-theta"
          type="number"
          v-model="props.sidebarStation.newStation.value.stationPosition.theta"
          :default-value="0"
        />
      </div>
    </div>
    ...
    <div class="grid gap-2 mt-4">
      ...
      <TagsInput
        ...
        :model-value="props.sidebarStation.nodeConnections.value"
        @update:model-value="
          props.sidebarStation.newStation.value.interactionNodeIds =
            props.sidebarStation.nodeConnections.value
        "
      >
        <div class="flex gap-2 flex-wrap items-center px-3">
          <TagsInputItem
            v-for="item in props.sidebarStation.nodeConnections.value"
            :key="item"
            :value="item"
          >
            <TagsInputItemText />
        ...
        <ComboboxRoot
          v-model="props.sideBarNode.nodeConnections.value"
          v-model:open="open"
          ...
          <ComboboxAnchor as-child>
            <ComboboxInput placeholder="Nodes.." as-child>
              <TagsInputInput
                class="w-full px-3"
                :class="
                  props.sidebarStation.nodeConnections.value.length > 0
                    ? 'mt-2'
                    : ''
                "
    ...
    <div class="ml-auto flex w-full space-x-2 mt-3">
      <Button
        variant="secondary"
        @click="
          props.layout.createStation(props.sidebarStation.newStation.value)
        "
        >Save</Button
      >
      <Button
        variant="secondary"
        :disabled="!props.sidebarStation.newStation.value.stationId"
        @click="
          props.sidebar.selectedStations.value = [];
          props.layout.deleteNode(
            props.sidebarStation.newStation.value.stationId,
          );
        "
        >Delete</Button
      >
